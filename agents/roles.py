...
_SCORE_SYS = (
    ...
    "strictly — reserve 5 for standouts, do not pile scores at 3. Do not "
    "compute a final total.\n"
    "\n"
    "Your reasoning must address all four dimensions by name — skills, "
    "seniority, domain, logistics. Judge seniority against the years/levels "
    "the posting asks, even when no profile skill matches (then say the "
    "candidate cannot meet that bar). When the posting states nothing about a "
    "dimension, say so and note what you judged it on (e.g. the role title "
    "alone); never silently skip a dimension and never invent detail the "
